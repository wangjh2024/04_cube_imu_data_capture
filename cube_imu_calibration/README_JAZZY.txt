...
如果直接在当前目录作为工作空间构建：
  ...
  cd /home/wangjh/code/06imu_cube_calib
  source /opt/ros/jazzy/setup.bash
...
界面流程：
  ...
  1. 确认图像话题、IMU 话题、CameraInfo 话题、Tag 边长、Cube 边长、Tag ID、bag 路径、录制时长和 IMU 串口。IMU 来源固定为 cube_serial，也就是 Cube 上的 /dev/ttyACM0 串口 IMU。
     ...
     Tag 黑色外框边长 0.0300 m，Cube 实测边长默认 0.0330 m，Tag ID=自动 0-4。
  ...
  5. 点击“一键开始采集”。GUI 会等待新鲜图像、IMU、CameraInfo 和 AprilTag 位姿全部就绪后再触发录制；默认录制 90 秒，进度条显示百分比，到时自动停止。
